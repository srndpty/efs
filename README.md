...
| lint | `.clang-tidy` (bugprone / performance / modernize / readability から実用的なものに限定)。実行は `pwsh scripts/lint.ps1` (ファイル単位で並列。`-Fix` で自動修正、`-Jobs N` で並列数) — CI も同じスクリプトを呼ぶ。**既定の clang-tidy は CI と同じ 22.1 系** (`-Bootstrap` で `.tidy22` に用意。版が違えば警告が出る) |
| テスト | QtTest + `ctest` |
...
**手編集するときは efs を一度終了させること** (トレイの `Quit` か
`efs.exe --quit`)。常駐したままだと、プロセスが起動時に読み込んだ古い値
(`hotkey/show` など) が終了時に書き戻され、手で書いた内容が消える。
なお保存は「書けたことを確認して」から成功とする (`sync()` +
`QSettings::status()`)。書けなかった場合は検索欄の下に
`Failed to save settings to …` を出し、トレイからも通知する — 閉じる = 隠す
なので、黙って捨てると設定が失われ続けることに気づけない。
...
| 対象外の版 | `Search failed: Unsupported Everything version 1.5.0.1391 (efs supports 1.4 only).` |
...
**接続先の Everything の版も見る (fail-closed)。** `regex:"…"` の引用、`<>` の
グルーピング、sort の意味論はどれも **1.4.1.1022 の実測**で確定した契約なので、
別系統の版に黙って別の意味で動かさせない。版が取れるのは IPC が通ってから
(未起動だと 0 が返る) なので、**最初に成功したクエリの直後**に `major.minor` を
確認し、`1.4` でなければ以後の検索をすべて上表の文言で失敗させる。
build number は gate にせずログに出すだけ (同じ 1.4 の別 build を弾く理由が無い)。

- **対象外と確定した後はクエリを投げない。** `search()` の冒頭で判定済みなら
  即失敗させる。毎回 5,000 件を読んでから捨てるのは無駄でしかない。
- **版が取れなかった場合 (0) は「対象外」として覚えない。** それは IPC が
  通っていないという意味 (クエリ成功と版の問い合わせの間に Everything が
  落ちた等) なので、その検索だけ `Everything is not running.` で失敗させ、
  判定は次回やり直す。latch すると Everything を起こし直しても永久に
  `Unsupported …` のままになる。
...
**`--tray` で隠して起動したときは、復元した種別での初回クエリを出さない。**
値 (種別 / Regex / ソート) は通常どおり復元し、クエリだけを最初の
`showAndActivate()` まで保留する (`SearchController::InitialDispatch::Deferred`)。
理由は 2 つ:

- 直前に `Image` や `Document` を使っていると、ログオンのたびに**誰も見ていない
  状態で数百万件の filter-only クエリ + 5,000 行のモデル/アイコン処理**が走る。
- ログオン時は efs が Everything 本体より先に起動しうる。その場合の初回クエリは
  `Everything is not running.` で失敗し、次の操作までその表示が残る。

通常起動 (`--tray` なし、またはトレイが使えない環境) は従来どおり即検索する。
...
- **配置先以外の場所から efs が常駐しているときは、何にも触らずに中止する。**
  single instance は「この PC で efs は 1 つ」を前提にしているので、旧
  `Program Files` 版が常駐したまま新しい場所へ入れると、実体とショートカットは
  新版なのに多重起動防止の primary は旧版のまま — 起動しても旧版が前へ出てくる。
  **見つけても kill はしない** (設定の保存を飛ばす)。実行中の path と `--quit`
  の手順を出して非ゼロで終わる。path を読めないプロセスも「別物ではない」と
  確認できていないので同じ扱い。
...
- **rollback 自体の失敗を成功扱いしない。** 復元は「旧版を戻せた」+「戻した中身が
  必須ファイルを満たす」+「退避した `.lnk` を戻せた」まで確認する。初回
  インストール (戻す旧版が無い) の経路も同じで、**partial な配置先を削除できた
  ことまで**見る — 消せなければ partial が「インストール済み」として残る。
  どれかが失敗したら、**staging / backup / ショートカットの退避を消さずに残し**、
...
- DLL 欠如時の診断が機能する: `Everything64.dll` の無いディレクトリから
  実行すると、探索した場所を報告する (起動失敗にはならない)。
  なお **P4 review で PATH へのフォールバックは廃止した** — 探索先は exe と
  同階層の絶対パス 1 つだけで、`LoadLibraryExW` の
  `LOAD_LIBRARY_SEARCH_SYSTEM32 | LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR` で
  依存 DLL の解決先も限定する。ビルド (`efs_copy_everything_dll`) と配布
  (`scripts/package.ps1`) の両方が DLL を exe の隣へ必ず置く契約なので、
  そこに無いのは配置の失敗であり、CWD/PATH 上の別の DLL を拾って
  「環境によって動いたり動かなかったりする」より明確に失敗する方が良い。
...
- 高速入力中も UI は固まらない (デバウンス 120 ms + 検索スレッド)。
  ...
  (計画 4)。**この待ち時間は当初 Regex で 7.7〜24.5 秒あったが、原因は Regex
  ではなく `EVERYTHING_REQUEST_ATTRIBUTES` だった** (「ATTRIBUTES を要求すると
  桁が変わる」を参照)。現在はどのクエリも 0.1 秒前後で返る。
...
### Regex 検索の実測 — 数十秒かかっていた (原因は Regex ではなかった。次節を参照)
...
> **後日訂正。** 「Everything の regex はインデックスを使えず全走査になる」という
> ここでの結論は誤り。真因は `EVERYTHING_REQUEST_ATTRIBUTES` で、Regex は無関係
> だった。同じ `Image` + `a12` は属性を要求しなければ **60 ms** で返る (下記)。

### `ATTRIBUTES` を要求すると桁が変わる — 要求フラグから外した

`Everything_SetRequestFlags()` に `EVERYTHING_REQUEST_ATTRIBUTES` を含めると、
**Everything がヒット 1 件ごとに属性をファイルシステムから読む**。1.4 は属性を
インデックスしないため、キャッシュに乗っていない初回クエリのコストがヒット件数に
比例して増える。2 回目以降が速いのはこのキャッシュのせいで、efs が速く見えていた
のは「同じ語を試した後」だったから。

初回 (未キャッシュ) の実測。SDK を直接叩き、efs と同じ呼び出し列を再現:

| クエリ | 総ヒット数 | `ATTRIBUTES` あり | なし |
|---|---:|---:|---:|
| `data` | 103,606 | 14,268 ms | 113 ms |
| `image` | 22,802 | 13,552 ms | 109 ms |
| `file` | 36,535 | 11,300 ms | 111 ms |
| `log` | 58,224 | 8,315 ms | 110 ms |
| `video` | 6,757 | 6,405 ms | 109 ms |
| `black` | 6,687 | 4,390 ms | 105 ms |
| `zebra` | 27 | 173 ms | 104 ms |
| `Image` + `regex:"a12"` | 3,655 | 26,189 ms | **60 ms** |

**属性を要求しなくても `IsFolderResult()` は正しく返る** (結果自体がフォルダ判定を
持っているため)。1,712 件で folder 判定 135 件が一致し、name / path / size /
modified も完全一致することを A/B で確認した。唯一の差はフォルダの size が
`0` ではなく取得失敗 (`-1`) になる点で、efs はフォルダの size を読まないので影響
しない。

したがって `search()` の要求フラグは `FILE_NAME | PATH | SIZE | DATE_MODIFIED` の
4 つだけにした。ヒット件数によらず初回 0.1 秒前後で返る (ヒット 150 万件の
`regex:"^[0-9]{3}"` でも 250 ms)。**独自インデックスは不要**。
