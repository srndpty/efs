...
- **`-Uninstall` が消すショートカットは、その `-Destination` を指しているものだけ。**
  `.lnk` の path は配置先に依らず同じ (ユーザープロファイル配下) なので、
  古い `Program Files` 側を消すつもりの `-Uninstall` が、生きている
  `%LOCALAPPDATA%` 側のショートカットを巻き添えにしないようにしてある。照合は
  実行中プロセスと同じく `<Destination>\efs.exe` との完全一致。
