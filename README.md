...
(未起動だと 0 が返る) なので、**最初に成功したクエリの直後に 1 度だけ**
`major.minor` を確認し、`1.4` でなければ以後の検索をすべて上表の文言で失敗させる。
...
- **rollback 自体の失敗を成功扱いしない。** 復元は「旧版を戻せた」+「戻した中身が
  必須ファイルを満たす」+「退避した `.lnk` を戻せた」まで確認する。どれかが
  失敗したら、**staging / backup / ショートカットの退避を消さずに残し**、
