...
//
// 本家 Everything と同じく、結果の Name / Path 列でクエリに一致した部分を
// 強調するための下ごしらえ。**描画は一切しない** — 返すのは文字位置だけで、
// Qt Core 以外に依存しない純粋なコードにしてある (単体テストの主戦場)。
//
// これは表示のための best-effort な近似であり、Everything の検索エンジンの
// authority ではない。Everything の全構文 (関数構文、`<>` のグルーピング、
// 演算子の優先順位設定) を再実装はしない — 一致しない部分が強調にならない
// ことはあっても、行そのものは正しく出る。多段のフォールバックは書かない
// (AGENTS.md「オーバーエンジニアリングを避ける」)。
#pragma once
...
public:
    ...
    //             照合器は空になる (強調が付かないだけ)。
    // Regex OFF — Everything の項区切り (空白 / `|`) で分解し、引用は外し、
    //             除外項 (`!...`) と関数構文 (`ext:jpg` 等) は落とす。
    //             残った項の `*` `?` はワイルドカードとして扱い、その場合だけ
    //             名前全体との一致を要求する (Everything と同じ)。
